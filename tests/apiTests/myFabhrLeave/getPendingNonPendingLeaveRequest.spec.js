...
test.describe("POST| -/hrmsApi/leaveApply/getPendingLeaveRequest/{flag}, Get Paginated Leave Pending and non pending Request Details API", () => {
  ...
  test("Get Paginated Leave Pending Request Details - Success scenario  @happy", async ({ request }) => {
     leave = new Leave();
    const requestBody = {
      ...payload.pagination1,
      companyId: companyId,
    ...
    const response = await leave.getPaginatedLeavePendingandNonpendingRequestDetails(
      request,
      authToken,
      "PEN",
      requestBody,
  ...
  test("Get Paginated Leave Non Pending Request Details - Success scenario  @happy", async ({ request }) => {
     leave = new Leave();
    const requestBody = {
      ...payload.pagination1,
      companyId: companyId,
  ...
  test("Get Paginated Leave Pending Request Details - Invalid pagination parameters  @negative", async ({ request }) => {
    leave = new Leave();
  ...
  test("Get Paginated Leave Pending Request Details - Without authentication token  @negative", async ({ request }) => {
    leave = new Leave();
    const requestBody = {
      ...payload.pagination1,
      companyId: companyId,
  ...
  test("Get Paginated Leave Pending Request Details - Different page sizes  @happy", async ({ request }) => {
    leave = new Leave();
    const testCases = [
  ...
  test("Get Paginated Leave Pending Request Details - Data shape and fields  @happy", async ({ request }) => {
    leave = new Leave();
    const requestBody = { ...payload.pagination1, companyId, employeeId };
    const response = await leave.getPaginatedLeavePendingandNonpendingRequestDetails(request, authToken, "PEN", requestBody);
    expect(response.status).toBe(200);
    const responseBody = response.body;
    expect(responseBody).toHaveProperty("data");
    expect(responseBody).toHaveProperty("currentPage");
  ...
  test("Get Paginated Leave Pending Request Details - All sorting scenarios  @happy", async ({ request }) => {
    leave = new Leave();

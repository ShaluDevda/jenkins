...
class Loan {
  async subMenuAllSubMenu(request, token, companyId) {
    const url = `${hrmsApi.hrmsApi}${endpoints.subMenuAllSubMenu}${companyId}`;
   ...
   async loanIssue(request, token, payload) {
    const url = `${hrmsApi.hrmsApi}${endpoints.loanIssue}`;
